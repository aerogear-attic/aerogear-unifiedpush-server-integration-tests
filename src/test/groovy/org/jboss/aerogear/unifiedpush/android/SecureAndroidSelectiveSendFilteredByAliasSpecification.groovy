...
@Mixin([AuthenticationUtils, PushApplicationUtils, AndroidVariantUtils,
    ...
    iOSVariantUtils, Constants])
class SecureAndroidSelectiveSendFilteredByAliasSpecification extends Specification {
    ...
    def static WebArchive "create deployment"() {
        Deployments.customUnifiedPushServerWithClasses(AndroidSelectiveSendFilteredByAliasSpecification.class, Constants.class)
    }
        ...
        Awaitility.await().atMost(Duration.FIVE_SECONDS).until(
                new Callable<Boolean>() {
                    public Boolean call() throws Exception {
                        return Sender.getGcmRegIdsList() != null && Sender.getGcmRegIdsList().size() == 2 // The condition that must be fulfilled
                    }
                }
                )
